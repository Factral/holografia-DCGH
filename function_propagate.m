...
% Nicolas Pegard, pegard@unc.edu, www.nicolaspegard.com
    ...
    if mod(M,2) == 1 % si es impar se le resta 1
        UY = 1:M; UY = UY - median(UY) - 1;
    else % si es par la media o la mediana quedan en x.5
        UY = 1:M; UY = UY - median(UY) - 0.5;   
    end
    
    if mod(N,2) == 1
        UX = 1:N; UX = UX - median(UX) - 1;
    else
        UX = 1:N; UX = UX - median(UX) - 0.5;    
    end
